...
from image_folder import SatData, DroneData
...
parser.add_argument('--dataset',default='university', type=str, help='university, cvusa, cvact')
...
parser.add_argument('--only_decouple', action='store_true', help='do not use balance' )
...
parser.add_argument('--pt', action='store_true', help='polar transform' )
...
image_datasets['satellite'] = SatData(data_dir, data_transforms['satellite'], data_transforms['train'], view='/streetview/', dataset=opt.dataset)
image_datasets['street'] = DroneData(data_dir, data_transforms['train'], data_transforms['satellite'], view='/streetview/', dataset=opt.dataset)
...
#            iter_ = 0
            ...
            for data,data2 in zip(dataloaders['satellite'], dataloaders['street']) :
                ...
                inputs, inputs_st, labels = data
                inputs2, inputs2_sa, labels2 = data2
                now_batch_size,c,h,w = inputs.shape
                if now_batch_size<opt.batchsize: # skip the last batch
                    continue
                if use_gpu:
                    inputs = Variable(inputs.cuda().detach())
                    inputs_st = Variable(inputs_st.cuda().detach())
                    inputs2 = Variable(inputs2.cuda().detach())
                    inputs2_sa = Variable(inputs2_sa.cuda().detach())
                ...
                if phase == 'val':
                    ...
                    if opt.views == 2: 
                        outputs, outputs2 = model(inputs, inputs2)
                        if opt.decouple:
                            outputs, outputs_f = outputs
                            outputs2, outputs2_f = outputs2
                        if opt.balance:
                            outputs2_sa, outputs_st = model(inputs2_sa, inputs_st)
                            if opt.decouple:
                                outputs2_sa, outputs2_sa_f = outputs2_sa
                                outputs_st, outputs_st_f = outputs_st
                ...
                if not opt.LPN and opt.pool != 'lpn':            
                    ...
                    if opt.views == 2:
                        if opt.balance:
                            if opt.only_decouple:
                                ...
                                loss1 = criterion(outputs, labels) + criterion(outputs2, labels2)
                                loss2 = criterion(outputs_st, labels) + criterion(outputs2_sa, labels2)
                                loss = (loss1 + loss2) / 2
                        else:
                            loss = criterion(outputs, labels) + criterion(outputs2, labels2)

                        if opt.decouple:
                            deloss1, on, off = decouple_loss(outputs_f, outputs_st_f, opt.scale, opt.lambd)
                            deloss2, on1, off1 = decouple_loss(outputs2_sa_f, outputs2_f, opt.scale, opt.lambd)
                    ...
                    if opt.balance:
                        if opt.only_decouple:
                            ...
                            preds, loss = one_LPN_output(outputs, labels, criterion, opt.block)
                            preds2, loss2 = one_LPN_output(outputs2, labels2, criterion, opt.block)
                            _, loss_1 = one_LPN_output(outputs_st, labels, criterion, opt.block)
                            _, loss2_1 = one_LPN_output(outputs2_sa, labels2, criterion, opt.block)
                            loss = (loss + loss_1) / 2
                            loss2 = (loss2 + loss2_1) / 2
                    else:
                        preds, loss = one_LPN_output(outputs, labels, criterion, opt.block)
                        preds2, loss2 = one_LPN_output(outputs2, labels2, criterion, opt.block)
...
if opt.views == 2:
    model = two_view_net(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool, share_weight=opt.share, VGG16=opt.use_vgg16, LPN=opt.LPN, block=opt.block, decouple=opt.decouple, swin=opt.swin)
    # if opt.LPN:
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
#record every run
    copyfile('./run_cvact.sh', dir_name+'/run_cvact.sh')
    copyfile('./train_cvact.py', dir_name+'/train_cvact.py')
