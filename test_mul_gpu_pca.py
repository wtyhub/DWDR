...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        query_number = query_feature.shape[0]
        All_feature = torch.cat((query_feature, gallery_feature), dim=0).numpy()
        #PCA
        pca = PCA(n_components=64)
        All_feature_pca = pca.fit_transform(All_feature)
        query_feature = All_feature_pca[:query_number]
        gallery_feature = All_feature_pca[query_number:]
        print('query_shape: ', query_feature.shape)
        print('gallery_shape: ', gallery_feature.shape)
    ...
    result = {'gallery_f':gallery_feature,'gallery_label':gallery_label,'gallery_path':gallery_path,'query_f':query_feature,'query_label':query_label, 'query_path':query_path}
