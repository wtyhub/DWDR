...
image_datasets['satellite'] = SatData(data_dir, data_transforms['satellite'], data_transforms['train'], view='/street/')
image_datasets['street'] = DroneData(data_dir, data_transforms['train'], data_transforms['satellite'], view='/street/')
...
if opt.views == 2:
    model = two_view_net(len(class_names), droprate=opt.droprate, stride=opt.stride, pool=opt.pool, share_weight=opt.share, VGG16=opt.use_vgg16, LPN=opt.LPN, block=opt.block, decouple=opt.decouple)
    # if opt.LPN:
