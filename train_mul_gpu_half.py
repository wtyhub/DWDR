...
def decouple_loss_half(y1, y2, scale_loss, lambd):
    batch_size, channel = y1.shape
    half = int(0.5 * channel)
    y1 = torch.split(y1, half, 1)[0].contiguous()
    y2 = torch.split(y2, half, 1)[0].contiguous()
    # print(y1.shape)
    c = y1.T @ y2
    c.div_(batch_size)
    on_diag = torch.diagonal(c)
...
def cal_corr_mat(x1, x2):
    batch_size = x1.size(0)
    y1 = cal_norm_val(x1)
    y2 = cal_norm_val(x2)
    corr = (y1.T @ y2) / (batch_size - 1)
    return corr

# def decouple_loss(y1, y2, scale_loss, lambd, alpha1=2, alpha2=2):
#     c = cal_corr_mat(y1, y2)
#     on_diag = torch.diagonal(c)
#     p_on = (1 - on_diag) / 2
#     on_loss = -1 * torch.pow(p_on, alpha1) * torch.log(1-p_on+1e-6)
#     on_loss = on_loss.mean()
#     off_diag = off_diagonal(c)
#     p_off = torch.abs(off_diag)
#     off_loss = -1 * torch.pow(p_off, alpha2) * torch.log(1-p_off+1e-6)
#     off_loss = off_loss.mean()
#     loss = on_loss + off_loss
#     return loss, on_loss, off_loss

# def decouple_loss(y1, y2, scale_loss, lambd, alpha1=0, alpha2=1):
#     # c = cal_corr_mat(y1, y2)
#     batch_size = y1.size(0)
#     c = y1.T @ y2
#     c.div_(batch_size)
...
#     p_on = (1 - on_diag) / 2
#     on_loss = -1 * torch.pow(p_on, alpha1) * torch.log(1-p_on+1e-6)
#     on_loss = on_loss.sum().mul(scale_loss)
#     off_diag = off_diagonal(c)
#     p_off = torch.abs(off_diag)
#     off_loss = -1 * torch.pow(p_off, alpha2) * torch.log(1-p_off+1e-6)
#     off_loss = torch.sort(off_loss, descending=True)[0]
#     end_index = math.floor(len(off_loss) * 0.6)
#     off_loss = off_loss[0:end_index]
#     off_loss = off_loss.sum().mul(scale_loss * lambd)
#     loss = 2*on_loss + off_loss
#     return loss, on_loss, off_loss
                    ...
                    if opt.decouple:
                        if opt.balance:
                            if opt.dro_lead:
                                ...
                                deloss1, on, off = decouple_loss_half(outs_f, outs_d_f, opt.scale, opt.lambd)
                                deloss2, on1, off1 = decouple_loss_half(outs3_s_f, outs3_f, opt.scale, opt.lambd)
                                deloss = (deloss1 + deloss2) / 2
                                # deloss = deloss2
                                on = (on + on1) / 2
                                off = (off + off1) / 2
