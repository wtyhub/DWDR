...
if opt.expand_id:
    ...
    image_datasets['satellite'] = AugSatData(data_dir, (data_transforms['satellite'],data_transforms['satellite_aug']), (data_transforms['train'], data_transforms['train_aug']))

if opt.select_id:
    print('--------------------select id-----------------------')
    image_datasets['drone'] = ImageFolder_selectID(os.path.join(data_dir, 'drone'), transform=data_transforms['train'])
else:
    image_datasets['drone'] = AugDroneData(data_dir, (data_transforms['train'],data_transforms['train_aug']), (data_transforms['satellite'],data_transforms['satellite_aug']))
...
def decouple_loss(y1, y2, scale_loss, lambd, w1=1, w2=1):
    batch_size = y1.size(0)
    c = y1.T @ y2
    c.div_(batch_size)
    ...
    loss = w1*on_diag + w2*off_diag * lambd
    return loss, on_diag, off_diag * lambd
    ...
    for epoch in range(num_epochs-start_epoch):
        ...
        for phase in ['train']:
            ...
            for data,data3 in zip(dataloaders['satellite'], dataloaders['drone']) :
                # get the inputs
                inputs, inputs_aug, inputs_d, inputs_d_aug, labels = data
                inputs3, inputs3_aug, inputs3_s, inputs3_s_aug, labels3 = data3
                ...
                if use_gpu:
                    if opt.normal:
                        ...
                        inputs_d_aug = Variable(inputs_d_aug.cuda().detach())
                        inputs3 = Variable(inputs3.cuda().detach())
                        inputs3_aug = Variable(inputs3_aug.cuda().detach())
                        inputs3_s = Variable(inputs3_s.cuda().detach())
                        inputs3_s_aug = Variable(inputs3_s_aug.cuda().detach())
                        labels = Variable(labels.cuda().detach())
                        labels3 = Variable(labels3.cuda().detach())
                ...
                if opt.balance:
                    if opt.decouple:
                        ...
                        aouts_d_c, aouts_d_f = model(inputs_d_aug)
                    else:
                ...
                if opt.balance:
                    if opt.decouple:
                        ...
                        aouts3_s_c, aouts3_s_f = model(inputs3_s_aug)
                    else:
                ...
                if opt.LPN:
                    ...
                    if opt.decouple:
                        if opt.balance:
                            ...
                            off = (off + off1) / 2
                            # intra
                            i_outs_f = torch.cat([outs_f, outs3_s_f], dim=0)
                            i_aouts_f = torch.cat([aouts_f, aouts3_s_f], dim=0)
                            i_outs3_f = torch.cat([outs3_f, outs_d_f], dim=0)
                            i_aouts3_f = torch.cat([aouts3_f, aouts_d_f], dim=0)
                            i_deloss1, i_on, i_off = decouple_loss(i_outs_f, i_aouts_f, opt.scale, opt.lambd)
                            i_deloss2, i_on1, i_off1 = decouple_loss(i_aouts3_f, i_outs3_f, opt.scale, opt.lambd)
                            i_deloss = (i_deloss1 + i_deloss2) / 2
                            i_on = (i_on + i_on1) / 2
                            i_off = (i_off + i_off1) / 2
                            deloss = deloss + i_deloss
            ...
            if opt.decouple:
                print('{} Loss: {:.4f} Satellite_Acc: {:.4f} Drone_Acc: {:.4f} Dec_Loss: {:.4f} On_Loss: {:.4f} Off_Loss: {:.4f}'
...
if not opt.resume:
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
#record every run
    copyfile('./run_intra_cross.sh', dir_name+'/run_intra_cross.sh')
    copyfile('./train_intra_cross.py', dir_name+'/train_intra_cross.py')
